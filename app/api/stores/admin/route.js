...
export async function GET(request) {
  try {
    ...
    if (storeIds.length > 0) {
      try {
        // Aggregate directly against the new storeId reference in the Product!
        productCounts = await Product.aggregate([
          { $match: { storeId: { $in: storeIds } } },
          { $group: { _id: '$storeId', count: { $sum: 1 } } },
        ]);
      } catch (aggError) {
        console.warn('Product aggregation failed, defaulting to 0:', aggError);
      }
    }
    ...
    const enrichedStores = stores.map((store) => {
      ...
      return {
        ...store,
        productCount:
          storeIdStr && countMap[storeIdStr] ? countMap[storeIdStr] : 0,
      };
    });
...
export async function POST(request) {
  try {
    ...
    if (error.code === 11000) {
      return NextResponse.json(
        { success: false, error: 'Store domain or title already exists' },
        { status: 400 },
      );
    }
    return NextResponse.json(
