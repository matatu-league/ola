/**
 * ============================================================================
 * FILE: app/api/stores/categories/[id]/route.js
...
export async function PUT(req, { params }) {
  try {
    // 1. Authenticate and get Store ID securely from the cookie
    ...
    const { id } = resolvedParams; // 'id' here is the Category ID
    ...
    // Ensure the category actually belongs to this specific store using the secure storeId
...
export async function DELETE(req, { params }) {
  try {
    ...
    const subcategories = await StoreCategory.find({ parentRef: id, storeRef: storeId });
    ...
    const deletedCategory = await StoreCategory.findOneAndDelete({ _id: id, storeRef: storeId });
