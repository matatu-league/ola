...
export const buildTemplatePrompt = ({
  ...
  if (bt === 'products')      siteBrief = productBrief;
  else if (isBoth)           siteBrief = `This business offers BOTH ${st || 'services'} AND physical products. Structure the site as a multi-view single-page app driven by tab-style VIEW STATE (see the SINGLE-PAGE, NO-ROUTING ARCHITECTURE):
   ...
   • A DEDICATED "Shop" / "Products" view — reached from a clearly-labelled top-nav item calling \`go('shop')\`, rendering the full PRODUCT grid (image, name, price, working search/filter, "Add to Cart"). Products are NOT shown on the services home page — they live on this dedicated shop view.
   • The top nav must let buyers move between the Services home (\`go('home')\`) and the Shop (\`go('shop')\`) (and the logo / a "Home" item returns to the services home). Clicking a product card opens its in-page detail view via \`go('product', id)\` (see the view-state contract below).`;
...
${command ? `\n=== HOUSE COMMAND (admin-defined for this industry — HIGH PRIORITY, follow it) ===\n${command}\n` : ''}
${modeDirective}

${isEditing ? `CRITICAL EDITING INSTRUCTION: The user wants to MODIFY their current design. I am providing the CURRENT SOURCE CODE below. Apply their requested changes specifically to this code without breaking existing logic, structure, animations or the data/navigation contracts. EVERY rule in this brief — ZERO ROUTING (no window.location/hash/history anywhere, views as tab state), tabs/dialogs for all switching, cart-as-drawer, checkout-as-the-only-handoff, tab-based custom pages, no ghost links — applies EQUALLY to an edit as to a fresh build. If the current code violates any of them (a hash router, a hashchange listener, any window.location read/write, a real page link, a rebuilt checkout), REWRITE that part to the view-state architecture as part of this edit even if the user didn't explicitly ask — preserving a broken pattern is not "preserving structure".\n\n--- CURRENT CODE ---\n${currentCode}\n--- END CURRENT CODE ---\n` : ''}
...
- Custom pages: ${business.pages && business.pages.length
    ? `this store has ${business.pages.length} custom page(s) — they are TABS, not routes: add a nav button + the single \`'page'\` view for EACH (see rule 5 below):\n${business.pages.map((p) => `  - "${p.title}" → go('page', '${p.slug}')`).join('\n')}`
    : 'none yet — omit any page nav buttons and the page view entirely.'}
...
3. IMAGES: use \`object-cover\`; product/room/service images should be a clean, consistent aspect ratio (wrap in a fixed-ratio container so cards never jump). Add \`loading="lazy"\` and descriptive \`alt\` text to every image, and ALWAYS include an inline SVG fallback for missing images (when \`!item.image\`) AND an \`onError\` that hides/replaces a broken \`<img>\`. For ALL decorative / hero / gallery / section photography use REAL keyword-based stock photos via LoremFlickr URLs of the form \`https://loremflickr.com/<width>/<height>/<comma,separated,industry,keywords>?lock=<number>\` (a fixed \`lock\` number keeps the same photo). Do NOT use source.unsplash.com (it is discontinued and 404s) and NEVER use AI-generated image services.
${ctaRule}
...
F. SMOOTH SCROLLING: for section navigation, scroll with \`document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' })\` from a button's onClick — never via \`<a href="#id">\` anchors (the zero-routing rule forbids hrefs).
...
2. Main component MUST be named \`App\` and MUST be a standard React arrow function.
   EXACT SYNTAX REQUIRED: \`const App = ({ storeName, storeLogo, storeBanner, contactEmail, contactPhone, categories, products, services, pages, businessType, serviceType, themeColor, apiBase }) => { ... }\`
...
DATA CONTRACT (props passed to App):
  storeName, storeLogo, storeBanner, contactEmail, contactPhone, categories, products, services, pages, businessType, serviceType, themeColor, apiBase
  ...
  - \`pages\`: array of vendor-authored custom pages, { title, slug, content }. \`content\` is plain text with real line breaks (render with \`whitespace-pre-line\`). This is the ONLY source of custom pages — see the CUSTOM PAGES rule below for how to route to them. If empty, render no page nav links and no page route at all.
...
The host injects a global \`window.__OLA__\` that connects this storefront to the REAL platform (shared cart, shared login, and the real themed checkout that writes orders to the database). You MUST route every cart and checkout action through it — do NOT build your own cart persistence or your own checkout/payment flow. Always feature-detect (\`window.__OLA__ && window.__OLA__.addToCart(...)\`) so the design still renders if it is ever absent.
  ...
  - \`__OLA__.viewCart()\` and \`__OLA__.viewProduct(id)\` exist but DO NOT use them — show the cart in the drawer and product detail in the in-page \`'product'\` view instead. Checkout is the only handoff.

=== SINGLE-PAGE ARCHITECTURE: NO ROUTING AT ALL · VIEWS ARE TAB STATE · DRAWER FOR CART · REAL SYSTEM CHECKOUT ===
This component is the storefront's ENTIRE experience (home, shop, product detail, custom pages) as ONE page, plus a slide-in CART DRAWER. Checkout is NOT rebuilt here — it is the real, theme-matched system checkout, reached via \`window.__OLA__.checkout()\`.
GOLDEN RULE — ZERO ROUTING: this component must NEVER touch the URL or navigation in any way. FORBIDDEN, no exceptions: \`window.location\` (read OR write), \`location.hash\`, \`location.href\`, \`hashchange\` listeners, \`history.pushState\`/\`replaceState\`/\`back\`, \`window.open\`, any \`<a href>\` that navigates (the ONLY tolerated hrefs are \`mailto:\` and \`tel:\`). Every "page" of the site — including the vendor's custom pages — is a TAB: a plain \`useState\` view switch where all content lives inside this one page. The SINGLE thing that ever leaves the page is the checkout handoff, \`window.__OLA__.checkout()\`, which the host bridge performs for you. This applies equally to fresh builds AND edits.
1. VIEW STATE (build this FIRST — it replaces any router). Hold the current view in plain state:
       \`const [view, setView] = useState({ name: 'home', id: null });\`
       \`const go = (name, id) => { setView({ name, id: id || null }); window.scrollTo(0, 0); };\`
   The ONLY view names — invent no others: \`'home'\` · \`'shop'\` (product listing) · \`'product'\` + id (product detail) · \`'page'\` + slug (one of the vendor's custom \`pages\`, only if \`pages\` is non-empty — see rule 5). Render exactly one view at a time by switching on \`view.name\`. ALL navigation — header nav items, logo → home, product cards, breadcrumbs, "back to shop", custom-page links — is a \`<button>\` (styled however the design needs) calling \`go(...)\`. There is NO 'cart' and NO 'checkout' view — the cart is a DRAWER and checkout is the system handoff.
...
3. NO GHOST / DEAD LINKS — and in fact NO \`<a href>\` AT ALL except \`mailto:\`/\`tel:\`. Forbidden: \`href="#"\`, \`href="#/anything"\`, \`href="javascript:void(0)"\`, empty \`href\`, server paths like \`/about\`/\`/login\`, fabricated social URLs, or any \`onClick\` that does nothing. Every interactive element MUST do something real: call \`go(...)\` to switch views, open/close the cart drawer, switch a tab, mutate the cart through \`__OLA__\`, call \`__OLA__.checkout()\`, select a variant, smooth-scroll to an id that ACTUALLY exists, or open an on-page modal you also render. If a conventional link has no real destination (Privacy, Terms, Instagram), render it as plain NON-interactive \`<span>\` text — never a clickable dead link. This applies to EVERY button.
4. TABS use the same approach (e.g. product-detail "Description / Specifications / Shipping" tabs). Keep the active tab in \`useState\` — NEVER in the URL.${bookingRule}
5. CUSTOM PAGES (\`pages\` prop). They are called "pages" but they are TABS — never independent pages/routes. If \`pages\` is non-empty, add ONE nav button per page (header and/or footer) calling \`go('page', slug)\`, and add a SINGLE \`'page'\` view (like the product-detail view finds its product by id) that looks up \`pages.find(p => p.slug === view.id)\`, then renders \`page.title\` as a heading and \`page.content\` as body copy with \`whitespace-pre-line\` — themed consistently with the rest of the site, not a bare unstyled block. If \`pages\` is empty, do NOT render any page nav buttons or the page view at all — this is what keeps rule 3 (no ghost links) satisfied; never fabricate an "About"/"FAQ" link that isn't backed by a real entry in \`pages\`.

=== PRODUCT DETAIL VIEW (the \`'product'\` view) — AS COMPLETE AS THE REAL SYSTEM PDP ===
Find the product by \`view.id\` from the \`products\` prop (if not found → a friendly "product not found" with a button back to the shop, \`go('shop')\`). Build a rich, conversion-grade detail view:
- BREADCRUMB: Home / Shop / <name> (crumbs are buttons calling \`go('home')\` / \`go('shop')\`; the current product crumb is plain text).
...
- RELATED: a "You may also like" grid mapping OTHER products (exclude the current id), each opening its own detail via \`go('product', id)\`.
...
- Product card / "View" → \`go('product', id)\` (the in-page detail view). NEVER a URL/route of any kind for browsing.
