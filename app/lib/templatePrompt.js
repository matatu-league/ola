...
${isEditing ? `CRITICAL EDITING INSTRUCTION: The user wants to MODIFY their current design. I am providing the CURRENT SOURCE CODE below. Apply their requested changes specifically to this code without breaking existing logic, structure, animations or the data/navigation contracts. EVERY rule in this brief — the single-page hash router, tabs/dialogs instead of new routes, cart-as-drawer, checkout-as-the-only-handoff, the custom-pages route, no ghost links — applies EQUALLY to an edit as to a fresh build. If the current code violates any of them (a real page link, a missing hash route, a rebuilt checkout, no pages route, etc.), FIX it as part of this edit even if the user didn't explicitly ask — preserving a broken pattern is not "preserving structure".\n\n--- CURRENT CODE ---\n${currentCode}\n--- END CURRENT CODE ---\n` : ''}
...
- Custom pages: ${business.pages && business.pages.length
    ? `this store has ${business.pages.length} custom page(s) — add a nav link + the \`#/page/<slug>\` view for EACH (see rule 5 above):\n${business.pages.map((p) => `  - "${p.title}" → #/page/${p.slug}`).join('\n')}`
    : 'none yet — omit any page nav links and the page route entirely.'}
...
GOLDEN RULE: nothing in this component may ever cause a full page reload or leave the page — with the SINGLE exception of the checkout handoff, which intentionally hands off to the real system. The 3 hash routes below are the only "views"; everything else that looks like switching content (product tabs, category filters, a filter panel, a quick-view, a confirmation) is either a TAB (swap what's shown, same view, plain \`useState\`) or a DIALOG/MODAL (a transient on-page overlay) — never a new route, never a link to a server page. Prefer adding a tab or a modal over inventing a new hash route.
1. HASH ROUTER (build this FIRST). Hold a route in state synced to \`window.location.hash\`; parse into \`{ view, id, tab }\`. The ONLY browsing routes — invent no others:
       \`#/\` → home · \`#/shop\` → product listing/shop · \`#/product/<id>\` → product detail · \`#/page/<slug>\` → one of the vendor's custom \`pages\` (only if \`pages\` is non-empty — see rule 8).
...
5. CUSTOM PAGES (\`pages\` prop). If \`pages\` is non-empty, add ONE nav link per page (header and/or footer) navigating to \`#/page/<slug>\`, and add a SINGLE \`#/page/<id>\` view (like the product-detail view finds its product by id) that looks up \`pages.find(p => p.slug === id)\`, then renders \`page.title\` as a heading and \`page.content\` as body copy with \`whitespace-pre-line\` — themed consistently with the rest of the site, not a bare unstyled block. If \`pages\` is empty, do NOT render any page links or the page view at all — this is what keeps rule 3 (no ghost links) satisfied; never fabricate an "About"/"FAQ" link that isn't backed by a real entry in \`pages\`.
