...
export function useAuth() {
    ...
    try {
      ...
      const res = await fetch('/api/auth/google', {
        ...
        body: JSON.stringify({ 
          ...
          phoneNumber: phoneNumber // Pass phone number to backend
        }),
      });
  ...
  const loginWithTikTok = async (phoneNumber) => {
    if (!phoneNumber) return alert("Phone number is required");
    setAuthProvider('tiktok');
    
    try {
      // Temporarily store phone number for the callback route to consume
      setWildcardCookie('pending_phone_number', phoneNumber, 3600); // 1 hour expiry
