...
export default function AccountStep({
  ...
  return (
    <div className={`bg-white border rounded-[var(--s-radius,0.125rem)] transition-all duration-300 ${isActive ? 'border-[var(--s-text,#161823)] ring-1 ring-[var(--s-primary,#161823)]' : 'border-[var(--s-border,#E3E3E4)]'}`}>
      {/* Header */}
      <div className={`p-4 md:p-5 flex items-center justify-between bg-white ${isActive ? 'border-b border-[var(--s-border,#E3E3E4)]' : ''} rounded-t-[var(--s-radius,0.125rem)]`}>
        ...
        <div className="p-5 md:p-6">
          ...
          <div className="flex flex-col sm:flex-row gap-2.5">
            <input
              ...
              className="flex-1 bg-white border border-[var(--s-border,#E3E3E4)] focus:border-[var(--s-text,#161823)] outline-none rounded-[var(--s-radius,0.125rem)] px-3.5 py-2.5 text-[14px] text-[var(--s-text,#161823)] transition-colors"
            />
            <button
              onClick={() => onSavePhone?.()}
              disabled={phoneSaving}
              className="inline-flex items-center justify-center gap-2 bg-[var(--s-primary,#161823)] text-[var(--s-on-primary,#fff)] hover:opacity-90 rounded-[var(--s-radius,0.125rem)] px-5 py-2.5 text-[13px] font-bold transition-opacity disabled:opacity-50 tracking-tight"
            >
      ...
      {isActive && !needsPhone && (
        <div className="p-5 md:p-6">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 w-full md:w-auto bg-white border border-[var(--s-border,#E3E3E4)] hover:bg-[var(--s-surface,#F8F8F8)] hover:border-[var(--s-text,#161823)] text-[var(--s-text,#161823)] rounded-[var(--s-radius,0.125rem)] px-5 py-2 text-[13px] font-semibold transition-colors disabled:opacity-50 tracking-tight"
          >
            {isAuthLoading ? (
      ...
      {/* Collapsed summary */}
      {isDone && user && (
        <div className="px-4 md:px-5 pb-4 pt-4 bg-white rounded-b-[var(--s-radius,0.125rem)] border-t border-[var(--s-border,#E3E3E4)] ml-9">
