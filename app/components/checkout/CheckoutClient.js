  ...
  if (cartItems.length === 0 && !isSubmitting) {
    return (
      <div className="max-w-[1200px] mx-auto px-4 py-20 flex flex-col items-center text-center bg-white">
        <div className="w-24 h-24 bg-[var(--s-surface,#F8F8F8)] border border-[var(--s-border,#E3E3E4)] rounded-[var(--s-radius,0.125rem)] flex items-center justify-center mb-6">
        ...
        <button onClick={() => router.push('/')} className="bg-[var(--s-primary,#161823)] hover:bg-black text-white px-6 py-2 rounded-[var(--s-radius,0.125rem)] font-semibold text-[13px] transition-colors tracking-tight">
          Continue Shopping
        </button>
      </div>
    );
    ...
    <div className="bg-white min-h-screen">
      <div className="max-w-[1200px] mx-auto px-4 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 pb-12 pt-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-4">
          ...
          {/* Global error banner */}
          {generalError && (
            <div className="flex items-center gap-2 text-[13px] text-[var(--s-primary,#FE2C55)] font-semibold bg-[#FFF0F3] border border-[var(--s-primary,#FE2C55)] p-4 rounded-[var(--s-radius,0.125rem)]">
