      ...
      <div className="max-w-[1400px] mx-auto px-4 mt-6 md:mt-8">
        <div className="flex flex-col gap-4">
          {stores.map((store) => {
            ...
            const storeUrl = store.domain ? `http://${store.domain}` : '#';
              ...
              <div key={store._id || store.id} className="bg-white rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow flex flex-col lg:flex-row gap-6 border border-transparent hover:border-gray-200">
                ...
                <div className="w-full lg:w-[35%] xl:w-[30%] flex flex-col">
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div>
                      {/* CHANGED: Use Link to navigate to the subdomain */}
                      <Link href={storeUrl}>
                        <h3 className="font-extrabold text-[15px] text-black hover:text-[#FE2C55] cursor-pointer underline-offset-2 hover:underline leading-snug mb-1">
                          {store.title}
                        </h3>
                      </Link>
                  ...
                  {/* Mobile Button - CHANGED to an anchor tag acting like a button */}
                  <div className="flex items-center gap-3 mt-auto lg:hidden pt-4">
                     <a href={storeUrl} className="flex-1 text-center rounded-full py-2 font-bold text-[13px] text-white transition-opacity hover:opacity-90 shadow-md block" style={{ backgroundColor: store.themeColor || '#000' }}>
                       Visit Store
                     </a>
                  </div>
                </div>
                ...
                <div className="w-full lg:w-[65%] xl:w-[70%] flex flex-col">
                   <div className="hidden lg:flex items-center justify-end gap-3 mb-4">
                     <a href={storeUrl} className="rounded-full px-8 py-2 font-bold text-[14px] text-white transition-transform hover:scale-105 shadow-md flex items-center gap-2" style={{ backgroundColor: store.themeColor || '#000' }}>
                   ...
                   <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 flex-1">
                     {store.products?.slice(0, 3).map((prod, i) => (
                       <Link href={`${storeUrl}/product/${prod._id}`} key={i} className="flex flex-col cursor-pointer group">
